...
class PlayerDV3(nn.Module):
    ...
    def get_actions(
        ...
        """
        actions, _, next_state = self._compute_actions(obs, greedy=greedy, mask=mask, state=None)
        self.restore_states(next_state)
        return actions

    def clone_states(self) -> "PlayerDV3.StateSnapshot":
    ...
    def restore_states(self, snapshot: "PlayerDV3.StateSnapshot") -> None:
    ...
    def _compute_actions(
    ...
    ) -> Tuple[Sequence[Tensor], Optional[Sequence[Tensor]], "PlayerDV3.StateSnapshot"]:
        if state is None:
        ...
        policy_logits = self.actor.get_logits(actor_input, mask) if return_logits else None
        actions_out, _ = self.actor(actor_input, greedy, mask)
        next_actions = torch.cat(actions_out, -1)
        next_state = self.StateSnapshot(next_actions, next_recurrent_state, next_stochastic_state)
        return actions_out, policy_logits, next_state
